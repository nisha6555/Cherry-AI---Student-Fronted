...
export function usePodcastPlayer({
  ...
  const handleDownloadTakeawaysAndTranscript = () => {
    ...
    const mentorName = podcast.hosts?.mentor?.name || "Mentor";
    const mentorRole = podcast.hosts?.mentor?.role || "Educator";
    const studentName = podcast.hosts?.student?.name || "Student";
    const studentRole = podcast.hosts?.student?.role || "Scholar";
    md += `**Hosts:** ${mentorName} (${mentorRole}) & ${studentName} (${studentRole})\n`;
