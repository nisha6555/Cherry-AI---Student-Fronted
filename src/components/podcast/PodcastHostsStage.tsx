...
export const PodcastHostsStage: React.FC<PodcastHostsStageProps> = ({
  ...
  const mentor = podcast.hosts?.mentor || { name: "Mentor Cherry", avatar: "👨‍🏫", title: "Lead Educator", voiceGender: "female" };
  const student = podcast.hosts?.student || { name: "Curious Scholar", avatar: "👩‍🎓", title: "Student Inquirer" };
      ...
      <div className="grid grid-cols-2 gap-3 relative z-10">
        ...
        >
          ...
          <span className="text-[9px] text-slate-400 font-mono mt-0.5 truncate max-w-[120px] px-1.5 py-0.5 rounded bg-slate-900/60 border border-slate-700/50">
            {(mentor.name || "").toLowerCase().includes("cherry") ||
            mentor.voiceGender === "female"
              ? "🎙️ Aoede (24kHz HD)"
              : "🎙️ Charon (24kHz HD)"}
          </span>
        </div>
