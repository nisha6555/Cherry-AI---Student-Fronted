...
export function computeReferralLevelStats(
  ...
  referrerTier: PlanReferralTier | undefined,
  commissionConfig: ReferralCommissionConfig
) {
  const level1Count = (refState.tierCounts && refState.tierCounts[1]) || 0;
  const level1EarnedFromActivities = (refState.activities || [])
    .filter((a) => a.level === 1 && a.status === "credited")
    .reduce((acc, curr) => acc + (curr.amount || 0), 0);
  const tier1Pct = referrerTier ? referrerTier.level1Percent : undefined;
  const reward1: number = tier1Pct
    ? Math.max(1, Math.round(149 * (tier1Pct / 100)))
    : (commissionConfig?.level1Reward ?? 0);
  const level1Earned =
    level1EarnedFromActivities > 0
      ? level1EarnedFromActivities
      : level1Count * (reward1 || 0);

  const level5Count = (refState.tierCounts && refState.tierCounts[5]) || 0;
  const level5EarnedFromActivities = (refState.activities || [])
    .filter((a) => a.level === 5 && a.status === "credited")
    .reduce((acc, curr) => acc + (curr.amount || 0), 0);
  const tier5Pct = referrerTier ? referrerTier.level5Percent : undefined;
  const reward5: number = tier5Pct
    ? Math.max(1, Math.round(149 * (tier5Pct / 100)))
    : (commissionConfig?.level5Reward ?? 0);
  const level5Earned =
    level5EarnedFromActivities > 0
      ? level5EarnedFromActivities
      : level5Count * (reward5 || 0);
