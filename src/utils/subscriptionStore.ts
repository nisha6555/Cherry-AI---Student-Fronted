...
export function buildDynamicUpiUri(
  ...
  if (typeof amountOrConfig === "object" && amountOrConfig !== null) {
    ...
    const tn = encodeURIComponent(amountOrConfig.note || `${upi.notes || "Subscription"} - ${amountOrConfig.transactionRef || orderId}`);
  ...
  const tn = encodeURIComponent(`${upi.notes || "Subscription"} - ${orderId}`);
...
export function extendStudentSubscription(
  ...
  const target = current.find((r) => r.studentId === studentId);
  if (!target) return null;
  const expMs = target.expiryDate ? new Date(target.expiryDate).getTime() : 0;
  const currExp = expMs > Date.now() ? new Date(target.expiryDate!) : new Date();
  ...
  if (typeof query === "string") {
    const found = subs.find(
      (s) =>
        ...
        s.status === "active" &&
        Boolean(s.expiryDate && new Date(s.expiryDate).getTime() > Date.now())
    );
...
export async function saveStudentSubscriptionToCloud(record: StudentSubscriptionRecord): Promise<boolean> {
  try {
    const docId = String(record.id || record.studentId || `sub_${Date.now()}`);
    const docRef = doc(db, "student_subscriptions", docId);
